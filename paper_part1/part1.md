...
усложняют поддержание производительности программного продукта и отладку
появляющихся проблем.

[Рисунок 1. Эволюция разработки приложений]{.image .placeholder
original-image-src="Picture_1.png"
original-image-title="fig:Рисунок 1. Эволюция разработки приложений"}
...
## Кратко о статье {#кратко_о_статье}
...
-- мониторинг, журналы событий и трассировка. Обсудим, как эти
...
можно упростить принятие решений при использовании Kubernetes в своих
проектах и продуктах.

Статья будет разбита на три части, в связи больших количеством
...
## Обзор сетевой инфраструктуры Kubernetes {#обзор_сетевой_инфраструктуры_kubernetes}
...
последовательную автоматизацию таких задач, как развертывание,
масштабирование и обновление контейнерных приложений. Системы
...
[Схема 1. Инфраструктура Kubernetes]{.image .placeholder
original-image-src="Scheme_1.png"
original-image-title="fig:Схема 1. Инфраструктура Kubernetes"}
...
идентификации подов
[1](https://kubernetes.io/docs/concepts/services-networking/)) и вместо
этого позволяет сетевым плагинам реализовывать свои собственные
настраиваемые сети.

Существует две основные категории сетевых плагинов. Первая - это плагин
...
набор функций для функционирования кластера Kubernetes. **Другая
категория** - сетевые плагины, реализующие спецификацию Container
Network Interface (CNI), называемые **CNI плагины**. Kubernetes не
зависит от конкретного используемого сетевого плагина (Flannel, Weave,
Calico, Cilium, Istio и др.), а плагины CNI могут реализовывать сетевую
модель с помощью совершенно разных подходов и предоставлять множество
дополнительных возможностей для инженеров Kubernetes. Также можно
настроить k8s с множеством CNI (например, связка Flannel + Calico, либо
Cilium + CNI для BGP протокола).

Сетевая инфраструктура Kubernetes также включает в себя сервисы
(services) и политики (policies), которые позволяют приложениям
взаимодействовать друг с другом внутри кластера.

**Service** Kubernetes обеспечивает постоянный IP-адрес и DNS-имя для
...
балансировку нагрузки между подами, что позволяет распределять трафик
между несколькими экземплярами приложения.

**Network Policy** Kubernetes позволяет управлять трафиком внутри
кластера, определяя правила доступа к сервисам и подам. Например,
политика может быть создана для разрешения доступа только из
определенного пространства имен или для определенных IP-адресов.
Политики также могут использоваться для настройки маршрутизации трафика,
например, для перенаправления трафика на новый сервис или для
перенаправления трафика на другой кластер.
...
сервисами запрещены при помощи политик.]{.image .placeholder
original-image-src="Picture_2.gif"
original-image-title="fig:Рисунок 2. Пример использования service и network policy. Трафик от внешних клиентов идёт только на сервис web, а сервис web общается с двумя другими сервисами foo и bar. Причем другие методы общения между сервисами запрещены при помощи политик."}
...
неисправностей сетевой инфраструктуры Kubernetes могут стать настоящей
головной болью. Чем более сложное взаимодействие между сервисами и
компонентами k8s пытается выстроить инженер, тем более вероятно
возникновение проблем, таких как блокировки сетевого трафика или
неожиданные задержки при работе приложений. А не использование сетевых
политик и защищенного трафика поставит под угрозу безопасность
разрабатываемого продукта.

Для более подробной информации о сетевой инфраструктуре Kubernetes вы
можете ознакомиться со [справочником
Kubernetes](https://kubernetes.io/docs/concepts/overview/components/#networking-components)
или использовать команду \`kubectl explain `<resource>`{=html}\` для
получения информации о конкретном ресурсе Kubernetes, например,
\`kubectl explain service\`.

`💡 Уже на этапе поднятия CNI для k8s можно осуществлять наблюдаемость за инфраструктурой при помощи встроенных инструментов от Kubernetes (Kubernetes Dashboard, kubectl, или получать информацию через  `[`Kubernetes API`](https://kubernetes.io/docs/reference/command-line-tools-reference/kube-apiserver/)`) либо при помощи инструментов от выбранных вами CNI (`[`Calico observability tools`](https://github.com/tigera-solutions/calicocloud-aks-workshop/blob/main/modules/using-observability-tools.md)`, `[`Cilium Hubble`](https://github.com/cilium/hubble#features)`, `[`Istio Kiali`](https://istio.io/latest/docs/tasks/observability/kiali/)`).`

### Пример микросервисной архитектуры в Kubernetes {#пример_микросервисной_архитектуры_в_kubernetes}

За основу для тестов и экспериментов в данной статье было выбрано
следующее
[демо](https://github.com/GoogleCloudPlatform/microservices-demo#readme)
...
той же).]{.image .placeholder original-image-src="Picture_3.png"
original-image-title="fig:Рисунок 3. Онлайн магазин товаров. В зависимости от версии, которую вы форкнете из репозитория, будут отличаться продаваемые товары (но это не должно вас смутить, так как архитектура взаимодействия сервисов остается той же)."}
...
[Схема 2. Архитектура приложения]{.image .placeholder
original-image-src="Scheme_2.png"
original-image-title="fig:Схема 2. Архитектура приложения"}
...
## eBPF и OpenTelemetry {#ebpf_и_opentelemetry}

**eBPF** (enhanced Berkeley Packet Filter) - это технология ядра Linux,
которая позволяет создавать и встраивать в ядро программы, которые могут
...
ресурсов. eBPF может также использоваться для создания пользовательских
...
<https://ebpf.io/applications/>.]{.image .placeholder
original-image-src="Picture_4.jpg"
original-image-title="fig:Рисунок 4. Сюда не вошли многие крупные проекты, которые также начинают адаптировать свои решения под eBPF (например, Istio). С полным списком можно ознакомиться на официальном сайте eBPF https://ebpf.io/applications/."}
...
инфраструктуры (инструментирование может быть как автоматическое, так и
требующее ручной установки). OpenTelemetry может быть использован для
мониторинга Kubernetes, включая мониторинг производительности,
отслеживания запросов и обнаружения ошибок (например, если агент на
...
делает его универсальным решением для сбора телеметрических данных из
различных типов приложений и систем.
...
OpenTelemetry Protocol.]{.image .placeholder
original-image-src="Picture_5.png"
original-image-title="fig:Рисунок 5. OpenTelemetry состоит из трёх основных компонент – формата данных, SDK для инструментирования приложений и коллектора для сбора, хранения и передачи данных. Связующим звеном является OTLP – OpenTelemetry Protocol."}

Обе технологии - eBPF и OpenTelemetry - имеют большой потенциал для
улучшения наблюдаемости Kubernetes и способны облегчить диагностику и
устранение проблем в микросервисной архитектуре.

[Таблица 1. Сценарии использования eBPF и OpenTelemetry для
наблюдаемости k8s.]{.image .placeholder original-image-src="Image6.png"
original-image-title="fig:Таблица 1. Сценарии использования eBPF и OpenTelemetry для наблюдаемости k8s."}

### Подробнее о eBPF {#подробнее_о_ebpf}

**Berkeley Packet Filter** (классический BPF) - это технология ядра
...
уровня приложений Linux на примере библиотек BCC/BPF для eBPF.]{.image
.placeholder original-image-src="Image7.png"
original-image-title="fig:Рисунок 6. Инструменты, сопоставленные с различными разделами ядра и уровня приложений Linux на примере библиотек BCC/BPF для eBPF."}
...
покрыть при помощи eBPF.]{.image .placeholder
original-image-src="Image8.png"
original-image-title="fig:Рисунок 7. При помощи каких хуков какую область сетевого стека можно покрыть при помощи eBPF."}
...
дальше по сетевому стеку). ]{.image .placeholder
original-image-src="Image9.png"
original-image-title="fig:Рисунок 8. Пример инструментирования сокетов (вместо трёх точек можно написать обработчик пакета, а при помощи флага TC_ACT_OK отправить его дальше по сетевому стеку). "}

#### Применимость eBPF в Kubernetes {#применимость_ebpf_в_kubernetes}
...
[Рисунок 9. Архитектура OpenTelemetry Collector]{.image .placeholder
original-image-src="Image8extraextra.png"
original-image-title="fig:Рисунок 9. Архитектура OpenTelemetry Collector"}
...
фреймворки]{.image .placeholder
original-image-src="Image8tripleextra.png"
original-image-title="fig:Таблица 2. Поддерживаемые OTel языки программирования и фреймворки"}

#### Эволюция архитектуры инструментирования приложений {#эволюция_архитектуры_инструментирования_приложений}

[Рисунок 10. Эволюция архитектуры инструментирования приложений]{.image
.placeholder original-image-src="Evolution_app_instrument_arch.png"
original-image-title="fig:Рисунок 10. Эволюция архитектуры инструментирования приложений"}
...
#### Применимость такого подхода в Kubernetes {#применимость_такого_подхода_в_kubernetes}
...
## Существующие подходы к мониторингу и наблюдаемости в Kubernetes {#существующие_подходы_к_мониторингу_и_наблюдаемости_в_kubernetes}
...
### Распределенная трассировка {#распределенная_трассировка}
...
распределенной трассы.]{.image .placeholder
original-image-src="Image11.png"
original-image-title="fig:Рисунок 11. Схема наблюдения за приложением в поде с использованием распределенной трассы."}
...
архитектуры магазина товаров: 1. Клиент начинает взаимодействие с
интерфейсом приложения.

[Рисунок 12. Один из примеров, как может выглядеть открытый клиентом
интерфейс приложения.]{.image .placeholder
original-image-src="Image12.png"
original-image-title="fig:Рисунок 12. Один из примеров, как может выглядеть открытый клиентом интерфейс приложения."}
...
архитектуры приложения]{.image .placeholder
original-image-src="Image13.png"
original-image-title="fig:Рисунок 13. Как выглядит распространение контекста в разрезе архитектуры приложения"}
...
пользователя.]{.image .placeholder original-image-src="Image14.png"
original-image-title="fig:Рисунок 14. Полученная распределенная трасса одного запроса от пользователя."}
...
[Рисунок 15. Сложность передачи метаданных при помощи eBPF.]{.image
.placeholder original-image-src="Image15.png"
original-image-title="fig:Рисунок 15. Сложность передачи метаданных при помощи eBPF."}

### Service Meshes {#service_meshes}
...
кластере для сбора сетевых данных.]{.image .placeholder
original-image-src="Image16.png"
original-image-title="fig:Рисунок 16. Схема наблюдения за K8s подом с использованием service mesh. Sidecar внедряет сетевые прокси между рабочими нагрузками в кластере для сбора сетевых данных."}
...
Cilium Service Mesh и Istio **Ambient** Mesh)]{.image .placeholder
original-image-src="Image17.png"
original-image-title="fig:Рисунок 17. Два сценария использования прокси: Proxy-per-pod модель, и Proxy-per-node proxy модель (один прокси на ноду, используется в моделях Cilium Service Mesh и Istio Ambient Mesh)"}
...
[Рисунок 18. Три типа моделей по обмену и сбору данных.]{.image
.placeholder original-image-src="Image18.png"
original-image-title="fig:Рисунок 18. Три типа моделей по обмену и сбору данных."}
...
### Системы мониторинга на основе eBPF {#системы_мониторинга_на_основе_ebpf}
...
хоста.]{.image .placeholder original-image-src="Image19.png"
original-image-title="fig:Рисунок 19. Наблюдение за подом в k8s с использованием eBPF. Программа eBPF перехватывает трафик пода при прохождении через сетевой стек хоста."}
...
[Связь между объектами: Kubernetes -- Cilium (CNI) -- Kernel]{.image
.placeholder original-image-src="Image20.png"
original-image-title="fig:Связь между объектами: Kubernetes – Cilium (CNI) – Kernel"}
...
узлов под управлением Istio. [Источник
данных](https://habr.com/ru/company/flant/blog/682520/).]{.image
.placeholder original-image-src="Picture_19.png"
original-image-title="fig:График 2. Влияние отключения Hubble в cilium-agent’е в кластере из ~30 узлов под управлением Istio. Источник данных."}
...
[Рисунок 20. Архитектура Pixie. PEM -- Pixie агенты на каждом узле,
Vizier -- коллектор для каждого кластера. Архитектуры других мониторинг
инструментов похожи на данную.]{.image .placeholder
original-image-src="Image21.png"
original-image-title="fig:Рисунок 20. Архитектура Pixie. PEM – Pixie агенты на каждом узле, Vizier – коллектор для каждого кластера. Архитектуры других мониторинг инструментов похожи на данную."}
...
[График 3. CPU FlameGraph для K8s подов.]{.image .placeholder
original-image-src="Image22.png"
original-image-title="fig:График 3. CPU FlameGraph для K8s подов."}
...
## Классы инструментов наблюдения за сетевой инфраструктуры Kubernetes {#классы_инструментов_наблюдения_за_сетевой_инфраструктуры_kubernetes}
...
kubectl-trace). - Service Mesh: Istio (+ Kiali UI), Linkerd. - CNI:
...
Coroot. - Точечные решение по сетевой и распределенной трассировке: PWRU
("paket, where are you?", сетевой трасса ядра), Odigos (распределенная
трасса приложений, дополненная информацией от eBPF). - Профайлеры (CPU,
user/kernel space вызовы и Network Performance): PyroScope, Parca,
Skywalking-rover, Kepler. - Инструменты для отладки: Inspektor-gadget,
Kubectl-trace.
...
уже будет дано развернутое описание: как разворачивались инструменты
(будут и другие инструменты не представленные в таблице ниже), как
проводилось тестирование, какие подводные камни встречались и какие
...
[Таблица 3. Сравнение инструментов наблюдаемости в k8s]{.image
.placeholder original-image-src="Table3.png"
original-image-title="fig:Таблица 3. Сравнение инструментов наблюдаемости в k8s"}
